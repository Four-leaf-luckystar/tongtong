(function () {
    ...
    let credentials = {};
    let state = { records: {}, conflicts: [], lastSyncAt: '', pending: false, error: '' };
    let timer = 0;
    ...
    function provider() {
        ...
        return {
            ...
            async upsert(record) { return request(safeUrl(settings.baseUrl, '/records/' + encodeURIComponent(record.id)), { method: 'PUT', headers: authHeaders(), body: json(record) }); },
            async remove(record) { await request(safeUrl(settings.baseUrl, '/records/' + encodeURIComponent(record.id)), { method: 'DELETE', headers: authHeaders() }); return true; }
        };
    }
    async function init() { if (initialized) return; initialized = true; const [s, c, st] = await Promise.all([read(SETTINGS_KEY), read(CREDENTIALS_KEY), read(STATE_KEY)]); settings = { ...settings, ...(s || {}), scope: { ...settings.scope, ...(s?.scope || {}) } }; credentials = c?.value || {}; state = { ...state, ...(st || {}), records: st?.records || {}, conflicts: Array.isArray(st?.conflicts) ? st.conflicts : [] }; }
        ...
        try {
            ...
            for (const currentBindingId of bindingIds) {
                ...
                for (const record of localGroup) {
                    ...
                    if (record.status === 'archived' || record.status === 'superseded') { if (remoteItem || previous) await adapter.remove({ remoteId: remoteItem?.remoteId || previous?.remoteId || record.id, id: record.id }); delete state.records[record.id]; continue; }
                    ...
                    await adapter.upsert(record, scope, remoteItem ? { remoteId: remoteItem.remoteId || remoteItem.id } : previous);
                    state.records[record.id] = { fingerprint: localFingerprint, remoteFingerprint: localFingerprint, remoteId: remoteItem?.remoteId || previous?.remoteId || record.id, remoteConfirmed: Boolean(remoteItem || previous?.remoteConfirmed), lastSyncedAt: new Date().toISOString() };
                }
